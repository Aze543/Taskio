...
const TaskioApp = () => {
  ...
  useEffect(() => {
    const fetch_tasks = async () => {
      try {
        const r = await fetch(
          `https://taskioapi.onrender.com/users/${user.id}`,
        )
        ...
        const createResponse = await fetch(
          "https://taskioapi.onrender.com/users",
          {
            method: "POST",
    ...
    if (task_tag === "Not Started") {
      const listTasks = tasks.map((task) =>
        task.id === task_id && task.user_id === user.id
          ? { ...task, tag: "Ongoing", type: "ongoing" }
          : task,
      )
      setTasks(listTasks)
      const myTask = listTasks.filter(
        (task) => task.id === task_id && task.user_id === user.id,
      )
      const updateOptions = {
        method: "PATCH",
        headers: {
      ...
      const updateOptions = {
        ...
        body: JSON.stringify({
          checked: myTask[0].checked,
          tag: myTask[0].tag,
          type: myTask[0].type,
          date_finished: myTask[0].date_finished,
        }),
      }
      const reqUrl = `${TASK_API}/${myTask[0].id}`
      const result = await apiRequest(reqUrl, updateOptions)
  ...
  return (
    <div className="my-3">
      ...
      <div className="container mt-5">
        ...
        {activeTab === "my-task" && (
          <MyTasks loading={isLoading} tasks={tasks} />
        )}
        {activeTab === "completed-task" && (
          <CompletedTasks loading={isLoading} tasks={tasks} />
        )}
      </div>
